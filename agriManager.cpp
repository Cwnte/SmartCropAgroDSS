...
void displayMenu() {
    ...
    std::cout << std::endl << "=== Nueva Ecija Offline Agri-DSS ===" << std::endl;
...
void loadFromTextFile(FarmSession& session) {
    std::ifstream file("C:\\Users\\PC\\source\\repos\\SmartCropAgroDSS\\INPUT_DATA\\database.txt");
...
void saveToTextFile(const FarmSession& session) {
    ...
    std::ofstream file("C:\\Users\\PC\\source\\repos\\SmartCropAgroDSS\\INPUT_DATA\\database.txt", std::ios::trunc);
